  ...
  useEffect(() => {
    ...
    getProductDetail(id)
      .then(res => {
        ...
        const allImages = [
          normalized?.commonImage && (normalized.commonImage.startsWith("http") ? normalized.commonImage : `${import.meta.env.VITE_API_URL?.replace("/swagger/index.html", "") || "https://weekly.eposh.io.vn"}/${normalized.commonImage}`),
          ...(normalized?.moreImages?.map(img => img.url && (img.url.startsWith("http") ? img.url : `${import.meta.env.VITE_API_URL?.replace("/swagger/index.html", "") || "https://weekly.eposh.io.vn"}/${img.url}`)) || [])
        ].filter(Boolean)
